...
- `src/data.ts` — ALL demo content, keyed by mode. `reviewQueues.product`:
  ...
  ids/files are `jasper-*`, naomi ×1 — her id/files are `julian-*`, renamed
  from Julian Long because the footage is a woman; avatar `naomi.png` is her
  julian-1 still cropped square via ffmpeg-static). Badges are a MIXED BAG
  per creator (emery Reel/TikTok/Story/Reel/TikTok, quinn Reel/Story/TikTok,
  carter Reel/Story, naomi Reel) — quinn's link-sticker checks/fills were
  reworded to bio-link on her non-Story clips. `reviewQueues.local`: Maya ×2 (IG
